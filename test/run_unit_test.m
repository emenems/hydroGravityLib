...
%% et0calc
% See http://www.fao.org/docrep/x0490e/x0490e08.htm#eto
% Test Daily global radiation input
...
if length(time_out)~= 1
    disp('et0calc daily (global): incorrect output length');
end
if round(ET0*100)/100 ~= 3.88
    disp('et0calc daily (global): incorrect output value');
end
clear time_out ET0
% Test Daily net radiaion input
[ET0,time_out] = et0calc('method','PM-FAO','temperature',[21.5 12.3]',...
 						'humidity',[84 63]','net_radiation',[13.28 13.28]',...
 						'radiation_units','MJ.m^-2','wind_speed',[10 10]',...
 						'wind_units','km.h^-1','wind_height',10,...
 						'time',[datenum(1999,7,6,2,30,0),datenum(1999,7,6,14,30,0)]',...
						'time_resolution','day',...
 						...
 						'Cn',900,'Cd',[0.34 NaN]);
if length(time_out)~= 1
    disp('et0calc daily (net): incorrect output length');
end
if round(ET0*100)/100 ~= 3.88
    disp('et0calc daily (net): incorrect output value');
end
clear time_out ET0
% Test hourly global radiation
...
if single(time_out(1)) ~= single(datenum(1999,10,1,3,00,0)) ||...
        length(time_out) ~= 2
    disp('et0calc hourly (global): incorrect output time/size');
end
if round(ET0(1)*100)/100 ~= 0 || round(ET0(2)*100)/100 ~= 0.63
    disp('et0calc hourly (global): incorrect output value');
end
clear time_out ET0
% Test hourly net radiation
[ET0,time_out] = et0calc('method','PM-FAO','temperature',[28 38]',...
						'humidity',[90 52]','net_radiation',[-0.5 1.749]',...
						'radiation_units','MJ.m^-2','wind_speed',[1.9 3.3]',...
						'wind_units','m.s^-1','wind_height',2,...
						'time',[datenum(1999,10,1,2,30,0),datenum(1999,10,1,14,30,0)]',...
						'time_resolution','hour',...
